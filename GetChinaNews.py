...
#"2015-11-01","2015-12-01",dirName='D:/testnews',timeLimit=20
...
if len(args) == 1:
	...
	if len(args) == 2:
		...
		if args[1] == "-a":	
			...
			print "help info of this commond:\nno arg: get news of today\n-a: get recent 30 days' news\n-d 2015-11-01: get news from the day\n-d 2015-11-01 2015-11-02: get news from the begin day to end day\n-dx 2015-11-01 t: get t days' news from the day\n"
			sys.exit(0)
	elif len(args) == 3:
		if args[1] != "-d":
			print "commond error,please see -help"
		...
		if args[1] != "-d" and args[1] != "-dx":
			...
			if args[1] == "-d":
				if not re.match(r"[\d]{4}-[\d]{2}-[\d]{2}$",args[3]):
					...
					sys.exit(0)
				#download -d 2015-11-01 2015-11-02：下区间内的
				argST = args[2]
				...
				if not re.match(r"[\d]*$",args[3]):
					...
					sys.exit(0)
				#download  -dx 2015-11-01 t：下指定日期以后t天的
				argST = args[2]
				datetmp = args[2].split('-')
				timetmp = time.mktime(datetime.datetime(int(datetmp[0]),int(datetmp[1]),int(datetmp[2])).timetuple())
